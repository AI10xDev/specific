...
pub const DEFAULT_SYSTEM_PROMPT: &str = "You are an inline text completion assistant. Return only a \
    ...
    brief phrase, or finish the current sentence. Never start a new line, continue with another sentence, \
    repeat the prefix, use Markdown, or add commentary. Prefer the shortest natural completion and return \
    only the characters to append.";
const COMPLETION_AGENT_SOURCE: &str = include_str!("../scripts/completion_agent.py");
const MAX_GHOST_COMPLETION_CHARS: usize = 160;
...
fn completion_env_file() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("KIBI_ENV_FILE").map(PathBuf::from) {
...
fn normalize_ghost_completion(completion: &str) -> Option<String> {
    let first_line = completion.trim_start_matches(['\r', '\n']).lines().next()?;
    let end = first_line
        .char_indices()
        .nth(MAX_GHOST_COMPLETION_CHARS)
        .map_or(first_line.len(), |(index, _)| index);
    let suffix = first_line[..end].trim_end();
...
fn append_save_history(file_name: &str, history_file: &Path) -> io::Result<PathBuf> {
    ...
    let canonical_path = fs::canonicalize(file_name)?;
    let mut history = OpenOptions::new().create(true).append(true).open(history_file)?;
    writeln!(history, "{}", canonical_path.display())?;
    Ok(canonical_path)
}

fn mirror_path(file_name: &str, copy_dir: &Path) -> Option<PathBuf> {
    let destination = Path::new(file_name).file_name().map(|name| copy_dir.join(name))?;
    (fs::canonicalize(file_name).ok().as_ref() != fs::canonicalize(&destination).ok().as_ref())
...
impl Editor {
    ...
    fn save_mirror(&self, file_name: &str, copy_dir: &Path) -> io::Result<Option<PathBuf>> {
        ...
        self.save(path.to_string_lossy().as_ref())?;
        Ok(Some(path))
    }
    ...
    fn start_completion_agent(&mut self) {
        ...
        cmd.args(["-c", COMPLETION_AGENT_SOURCE]).stdin(Stdio::piped()).stdout(Stdio::piped());

        if let Some(env_file) = completion_env_file() {
        ...
        loop {
            ...
            if let Some(receiver) = &self.completion_receiver {
                ...
                if let Some(completion) = latest
                    && self.last_completion_req_pos == Some((self.cursor.x, self.cursor.y))
                {
                    if completion.starts_with("Error:") {
                        set_status!(self, "Completion {}", completion);
                        self.refresh_screen()?;
                    } else if let Some(suffix) = normalize_ghost_completion(&completion) {
    ...
    fn save_and_handle_io_errors(&mut self, file_name: &str) -> bool {
        let history_file = std::env::var_os("KIBI_SAVE_HISTORY_FILE").map(PathBuf::from);
        let copy_dir = std::env::var_os("KIBI_SAVE_COPY_DIR").map(PathBuf::from);
        self.save_with_destinations(file_name, history_file.as_deref(), copy_dir.as_deref())
    }

    fn save_with_destinations(
        &mut self, file_name: &str, history_file: Option<&Path>, copy_dir: Option<&Path>,
    ) -> bool {
        ...
        match saved.as_ref() {
            Ok(w) => {
                let history_error =
                    history_file.and_then(|path| append_save_history(file_name, path).err());
                let mirror_result =
                    copy_dir.map_or(Ok(None), |dir| self.save_mirror(file_name, dir));

                match (mirror_result, history_error) {
                    (Err(mirror_err), Some(history_err)) => set_status!(
                        self,
                        "Saved to {file_name}, but history and mirror failed: {history_err}; {mirror_err}"
                    ),
                    (Err(err), None) => {
                        set_status!(self, "Saved to {file_name}, but mirror failed: {err}");
                    }
                    (Ok(Some(path)), None) => set_status!(
                        self,
                        "{} written to {} and {}",
                        format_size(*w as u64),
                        file_name,
                        path.display()
                    ),
                    (Ok(_), Some(err)) => {
                        set_status!(self, "Saved to {file_name}, but history update failed: {err}");
                    }
                    (Ok(None), None) => {
                        set_status!(self, "{} written to {}", format_size(*w as u64), file_name);
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn ghost_completion_is_rendered_and_accepted() -> Result<(), Error> {
        let mut editor = Editor {
        ...
        editor.insert_str("fn add");
        editor.ghost_text = normalize_ghost_completion("(a: i32, b: i32) -> i32\n{ a + b }");

        let mut output = String::new();
        editor.draw_rows(&mut output)?;
        assert!(output.contains("fn add(a: i32, b: i32) -> i32"));

        editor.process_keypress(&Key::Char(b'\t'));
        assert_eq!(editor.rows[0].chars, b"fn add(a: i32, b: i32) -> i32");
        assert!(editor.ghost_text.is_none());
        Ok(())
    }

    #[test]
    fn ghost_completion_is_limited_to_a_short_inline_suffix() {
        assert_eq!(
            normalize_ghost_completion("\n naturally finishes here.\nA second sentence."),
            Some(" naturally finishes here.".to_owned())
        );
        assert_eq!(normalize_ghost_completion("\n\r"), None);

        let long = "x".repeat(MAX_GHOST_COMPLETION_CHARS + 20);
        assert_eq!(
    ...
    #[test]
    fn save_history_creates_parent_and_appends_canonical_paths() -> Result<(), io::Error> {
        let temp = TempDir::new()?;
        let source = temp.path().join("source").join("feature.md");
        fs::create_dir_all(source.parent().expect("source has a parent"))?;
        fs::write(&source, "feature")?;
        let history = temp.path().join("state").join("specific").join("saved-files");

        let canonical = append_save_history(source.to_str().expect("UTF-8 path"), &history)?;
        append_save_history(source.to_str().expect("UTF-8 path"), &history)?;

        let expected = format!("{}\n{}\n", canonical.display(), canonical.display());
        assert_eq!(fs::read_to_string(history)?, expected);
        Ok(())
    }

    #[test]
    fn mirror_uses_basename_and_avoids_self_copy() -> Result<(), io::Error> {
        let temp = TempDir::new()?;
        let source_dir = temp.path().join("source");
        let source = source_dir.join("feature.md");
        fs::create_dir_all(&source_dir)?;
        fs::write(&source, "feature")?;

        assert_eq!(
            mirror_path(source.to_str().expect("UTF-8 path"), &temp.path().join("mirror")),
            Some(temp.path().join("mirror").join("feature.md"))
        );
        assert_eq!(mirror_path(source.to_str().expect("UTF-8 path"), &source_dir), None);
        Ok(())
    }

    #[test]
    fn mirror_directory_is_created_and_original_save_survives_mirror_error() -> Result<(), Error> {
        let temp = TempDir::new()?;
        let source = temp.path().join("feature.md");
        let mirror = temp.path().join("nested").join("mirror").join("feature.md");
        let mut editor = Editor { rows: vec![Row::new(b"feature".to_vec())], ..Default::default() };

        editor.save(source.to_str().expect("UTF-8 path"))?;
        assert_eq!(
            editor.save_mirror(
                source.to_str().expect("UTF-8 path"),
                &temp.path().join("nested").join("mirror"),
            )?,
            Some(mirror.clone())
        );
        assert_eq!(fs::read_to_string(&mirror)?, "feature");

        let invalid_mirror = temp.path().join("not-a-directory");
        fs::write(&invalid_mirror, "blocking file")?;
        editor.dirty = true;
        assert!(editor.save_with_destinations(
            source.to_str().expect("UTF-8 path"),
            None,
            Some(&invalid_mirror)
        ));
        assert!(!editor.dirty);
        assert!(
            editor.status_msg.as_ref().is_some_and(|status| status.msg.contains("mirror failed"))
        );
        assert_eq!(fs::read_to_string(source)?, "feature");
        Ok(())
    }
}
