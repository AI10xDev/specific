...
AI completion reads Azure OpenAI configuration from the process environment or
the nearest `.env` file in the working directory or executable path. Set
`KIBI_ENV_FILE` to use a specific file. The required values are
`AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY`, and `DEPLOYMENT_NAME`.
Legacy Azure endpoints also require `AZURE_OPENAI_API_VERSION`.
...
The parent `specific` project installs this binary and provides its shell
integration. See the repository-level README for portable installation.
